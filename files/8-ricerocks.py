...
def draw(canvas):
    ...
    #draws and update rocks and missiles
...
def rock_spawner():
    ...
    rock_vel = [(random.random() * .6 - .3) * (score / 75), (random.random() * .6 - .3) * (score / 75)]
    rock_avel = random.random() * .2 - .1
    #a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if len(rock_group) < 12:
        if dist(my_ship.get_pos(), rock_pos) > 100:
